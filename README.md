...
### 3. 프로필 실시간 업데이트 하기
```
프로필을 실시간으로 업데이트하기 위한 방법으로는 컴포넌트를 리렌더링 하는 방법으로 구현할수 있음
리렌더링은 두가지 경우에 발생함
1. useState로 관리중인 상태가 업데이트 되는경우
2. 프롭스로 받은 요소가 업데이트 되는 경우
```
* userObj는 스스로 업데이트 된 프로필ㅇㄹ 반영하지 못함
* 이를 위해 새 프로필을 파이어베이스에서 받아온다음 userObj에 반영해야함
* 즉, userObj를 새로고침하는 함수를 만들어줘야함
### 4. userObj 크기 줄이기
```
리렌더링이 안되는 이상한 현상이 생감
또 새로 고침을 해야 반영됨
원래 상태나 프롭스의 내용이 업데이트 되면 리렌더링이 발생해야함.
하지만 리액트는 상태나 프롭스의 내용물이 너무 많으면 그 안에있는 작은 변화를 제대로 인식하지 못함
그래서 userObj의 크기를 줄여야해!
```
## `[6월10일]`
### 1. 파이어베이스 스토리지 임포트하기
``` 
